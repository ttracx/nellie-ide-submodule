...
description: Nellie IDE is the open-source autopilot for software development and a fork of Continue.
...
- `CMD+L` - New chat. `CMD+SHIFT+L` - Append to current chat.
  ...
  - Address sign commands
    ...
    - All the other address sign commands are listed in this [doc](https://github.com/ttracx/nellie-ide/customization/context-providers). We want them to be indicated on one bullet point each, so they can be easily seen as a list. In the doc linked, they're too separated, requiring too much scrolling.
  ...
  - Slash commands
    ...
    - Include the [excerpt](https://github.com/ttracx/nellie-ide/customization/slash-commands#custom-slash-commands) about custom slash commands
