...
export interface IDE {
  ...
  updateNellieCredentials(auth: PearAuth): Promise<void>;
...
export interface Config {
  /** If set to true, Nellie IDE will collect anonymous usage data to improve the product. If set to false, we will collect nothing. Read here to learn more: https://github.com/ttracx/nellie-ide/telemetry */
  allowAnonymousTelemetry?: boolean;
   ...
   * Nellie IDE will do the rest of the work to construct prompt templates, handle context items, prune context, etc.
   */
   ...
   * A CustomContextProvider requires you only to define a title and getContextItems function. When you type '@title <query>', Nellie IDE will call `getContextItems(query)`.
   */
  contextProviders?: (CustomContextProvider | ContextProviderWithParams)[];
  /** If set to true, Nellie IDE will not index your codebase for retrieval */
  disableIndexing?: boolean;
  /** If set to true, Nellie IDE will not make extra requests to the LLM to generate a summary title of each session. */
  disableSessionTitles?: boolean;
  /** An optional token to identify a user. Not used by Nellie IDE unless you write custom coniguration that requires such a token */
  userToken?: string;
  /** The provider used to calculate embeddings. If left empty, Nellie IDE will use transformers.js to calculate the embeddings with all-MiniLM-L6-v2 */
  embeddingsProvider?: EmbeddingsProviderDescription | EmbeddingsProvider;
  /** The model that Nellie IDE will use for tab autocompletions. */
...
// in the actual Nellie IDE source code
 ...
 */
export interface INellieCreatorMode {
  /**
...
export type NellieCreatorModeMessage = {
  messageId: string;
} & (SubmitIdeaType | ProcessLLMType | CloseMessageType);

export type NellieCreatorSavedGlobalState =
  | {
      msg: SubmitIdeaType;
...
export interface INellieApi {
  readonly creatorMode: INellieCreatorMode;
  getUserId(): Promise<string | undefined>;
}

export interface NellieExtensionExports {
  pearAPI: INellieApi;
