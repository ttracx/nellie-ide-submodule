...
describe("walkDir", () => {
  ...
  test("should listen to both .gitignore and .nellieignore", async () => {
    const files = [
      [".gitignore", "*.py"],
      [".nellieignore", "*.ts"],
      "a.txt",
  ...
  test("should walk continue repo without getting any files of the default ignore types", async () => {
    const results = await walkDir(path.join(__dirname, ".."), ide, {
      ignoreFiles: [".gitignore", ".nellieignore"],
    });
  ...
  test.skip("should walk continue/extensions/vscode without getting any files in the .nellieignore", async () => {
