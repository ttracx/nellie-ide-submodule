...
function resolveSerializedConfig(filepath: string): SerializedContinueConfig {
  ...
  // Replace "nellie-server" with "nellie_server" at the beginning
  // This is to make v0.0.3 backwards compatible with v0.0.2
  content = content.replace(/"nellie-server"/g, '"nellie_server"');
  ...
  for (const desc of config.models) {
    if (isModelDescription(desc)) {
      const llm = await llmFromDescription(
        ...
        async (auth: PearAuth) => await ide.updateNellieCredentials(auth),
      );
      if (!llm) {
        continue;
      }
...
async function buildConfigTs() {
  ...
  try {
    ...
    console.log(
      `Build error. Please check your ~/.nellie/config.ts file: ${e}`,
    );
    return undefined;
  }
...
const STATIC_MODELS: ModelDescription[] = [
  {
    ...
    title: "Nellie IDE Model (Recommended)",
  ...
  {
    model: "perplexity",
    title: "Nellie IDE Search (Powered by Perplexity)",
