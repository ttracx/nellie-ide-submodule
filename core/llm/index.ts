...
import { anonymousTelemetryLog } from "../nellieServer/util.js";
...
${prompt}`;
  ...
  async *streamChat(
    ...
    // Privacy Policy: https://github.com/ttracx/nellie-ide/privacy-app - We send this anonymous data to our servers to help us improve the product and check for upstream security issues.
    // No actual contents of the user's request are sent in this call.
    // You can opt-out of this by setting sendAnonymousTelemetry to false in your config.json in ~/.nellie
    try {
      if (Telemetry.allow) {
