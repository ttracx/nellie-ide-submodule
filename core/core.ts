...
import NellieServer from "./llm/llms/NellieServer";
...
export class Core {
  ...
  constructor(
    ...
    on("llm/setNellieCredentials", async (msg) => {
      const { accessToken, refreshToken } = msg.data || {};
      const config = await this.configHandler.loadConfig();
      const pearAIModels = config.models.filter(
        (model) => model instanceof NellieServer,
      ) as NellieServer[];

      try {
        if (pearAIModels.length > 0) {
          pearAIModels.forEach((model) => {
            model.setNellieAccessToken(accessToken);
            model.setNellieRefreshToken(refreshToken);
          });
        }
      } catch (e) {
        console.warn(`Error resetting Nellie IDE credentials: ${e}`);
        return undefined;
      }
    });

    on("llm/checkNellieTokens", async (msg) => {
      const config = await this.configHandler.loadConfig();
      const pearAIModels = config.models.filter(
        (model) => model instanceof NellieServer,
      ) as NellieServer[];
      ...
      try {
        ...
        console.warn(`Error checking Nellie IDE tokens: ${e}`);
