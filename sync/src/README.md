...
- Only handles local files, so is not currently being used in situations where the Nellie IDE server is on a different machine from the IDE or the workspace (Remote SSH, WSL, or a Nellie IDE server being run for a team).
- Currently not using stat to check for recent changes to files, is instaed re-calculating the entire Merkle tree on every IDE reload. This is fine for now since it only takes 0.2 seconds on the Nellie IDE codebase, but is a quick improvement we can make later.
