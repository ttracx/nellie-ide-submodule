...
export class VerticalPerLineDiffManager {
  ...
  clearForFilepath(filepath: string | undefined, accept: boolean) {
    ...
    vscode.commands.executeCommand("setContext", "nellie.diffVisible", false);
  }
  ...
  async streamEdit(
    ...
    vscode.commands.executeCommand("setContext", "nellie.diffVisible", true);
    ...
    vscode.commands.executeCommand("setContext", "nellie.streamingDiff", true);
    ...
    try {
      ...
      if (error.message.includes("401")) {
        vscode.window
          .showErrorMessage(
            `Nellie IDE access requires login: Fetch Error: Invalid response body while trying to fetch Nellie IDE Server`,
            "Login to Nellie IDE",
          )
          .then((selection) => {
