...
import { PEARAI_CHAT_VIEW_ID, PEARAI_CREATOR_VIEW_ID, PEARAI_MEM0_VIEW_ID, PEARAI_SEARCH_VIEW_ID } from "../util/nellie/nellieViewTypes";
...
export class VsCodeExtension {
  ...
  constructor(context: vscode.ExtensionContext) {
    ...
    const outputChannel = vscode.window.createOutputChannel("Nellie IDE");
    ...
    context.subscriptions.push(
      vscode.window.registerUriHandler({
        handleUri(uri: vscode.Uri) {
          ...
          if (uri.authority === "nellie.nellie") {
            if (uri.path === "/ping") {
              vscode.window.showInformationMessage(
                "Nellie IDE received a custom URI!",
              );
              ...
              vscode.commands.executeCommand("nellie.updateUserAuth", data);
            }
          }
        },
      }),
    ...
    vscode.workspace.onDidSaveTextDocument(async (event) => {
      ...
      if (filepath.endsWith(".nellierc.json") || filepath.endsWith(".prompt")) {
        this.configHandler.reloadConfig();
      } else if (
        filepath.endsWith(".nellieignore") ||
  ...
  static continueVirtualDocumentScheme = "nellie";
