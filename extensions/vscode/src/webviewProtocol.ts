  ...
  implements IMessenger<FromWebviewProtocol, ToWebviewProtocol>
{
  ...
  resetWebviewToDefault() {
    const defaultViewKey = "nellie.chatView";
        ...
        try {
          ...
          if (e.cause) {
            if (e.cause.name === "ConnectTimeoutError") {
              message = `Connection timed out. If you expect it to take a long time to connect, you can increase the timeout in config.json by setting "requestOptions": { "timeout": 10000 }. You can find the full config reference here: https://github.com/ttracx/nellie-ide/reference/config`;
            } else if (e.cause.code === "ECONNREFUSED") {
              message = `Connection was refused. This likely means that there is no server running at the specified URL. If you are running your own server you may need to set the "apiBase" parameter in config.json. For example, you can set up an OpenAI-compatible server like here: https://github.com/ttracx/nellie-ide/reference/Model%20Providers/openai#openai-compatible-servers--apis`;
            } else {
              message = `The request failed with "${e.cause.name}": ${e.cause.message}. If you're having trouble setting up Nellie IDE, please see the troubleshooting guide for help.`;
            }
          }
          // Nellie IDE login issues
          else if (message.includes("401") && message.includes("Nellie IDE")) {
            vscode.window
              .showErrorMessage(
                message,
                'Login To Nellie IDE',
              ...
              .then((selection) => {
                if (selection === 'Login To Nellie IDE') {
                  // Redirect to auth login URL
                  vscode.env.openExternal(
                    vscode.Uri.parse(
                      'https://github.com/ttracx/nellie-ide/signin?callback=nellie://nellie.nellie/auth',
                    ),
                  );
          ...
          // Nellie IDE Free trial ended case
          else if (message.includes("403") && message.includes("Nellie IDE")) {
            vscode.window
              .showErrorMessage(
                message,
                'View Nellie IDE Pricing',
                'Show Logs',
              )
              .then((selection) => {
                if (selection === 'View Nellie IDE Pricing') {
                  // Redirect to auth login URL
                  vscode.env.openExternal(
                    vscode.Uri.parse(
                      'https://github.com/ttracx/nellie-ide/pricing',
                    ),
                  );
                } else if (selection === 'Show Logs') {
                  vscode.commands.executeCommand(
            ...
            if (message.includes("exceeded")) {
              message +=
                " To keep using Nellie IDE, you can set up a local model or use your own API key.";
            }
            ...
            vscode.window
              ...
              .then((selection) => {
                if (selection === "Show Logs") {
                  ...
                  vscode.env.openExternal(
                    vscode.Uri.parse(
                      "https://github.com/ttracx/nellie-ide/troubleshooting",
                    ),
                  );
                }
              });
