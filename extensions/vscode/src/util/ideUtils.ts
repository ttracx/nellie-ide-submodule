...
export class VsCodeIdeUtils {
  ...
  async getUserSecret(key: string) {
    ...
    let secret = vscode.workspace.getConfiguration("nellie").get(key);
    ...
    secret = await vscode.window.showInputBox({
      prompt: `Either enter secret for ${key} or press enter to try Nellie IDE for free.`,
      password: true,
    });
  ...
  async getPearCredentials(): Promise<PearAuth> {
    return await vscode.commands.executeCommand("nellie.getPearAuth");
  }
   ...
   */
  async updateNellieCredentials(auth: PearAuth) {
    await vscode.commands.executeCommand("nellie.updateUserAuth", auth);
  }
}
