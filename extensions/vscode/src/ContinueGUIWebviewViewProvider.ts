...
import { PEARAI_CHAT_VIEW_ID, PEARAI_OVERLAY_VIEW_ID } from "./util/nellie/nellieViewTypes";

// The overlay's webview title/id is defined in nellie-app's PearOverlayParts.ts
  ...
  implements vscode.WebviewViewProvider
{
  ...
  private updateDebugLogsStatus() {
    const settings = vscode.workspace.getConfiguration("nellie");
  ...
  private setupDebugLogsListener() {
    vscode.workspace.onDidChangeConfiguration((event) => {
      if (event.affectsConfiguration("nellie.enableDebugLogs")) {
        const settings = vscode.workspace.getConfiguration("nellie");
  ...
  constructor(
    ...
    this.outputChannel = vscode.window.createOutputChannel("Nellie IDE");
  ...
  getSidebarContent(
    ...
    const panelViewType = panel?.viewType; // eg. nellie.chatView
    const isOverlay = panel?.title === PEARAI_OVERLAY_VIEW_ID; // defined in nellie-app PearOverlayPart.ts
