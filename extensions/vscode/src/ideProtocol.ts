...
class VsCodeIde implements IDE {
  ...
  getIdeInfo(): Promise<IdeInfo> {
    return Promise.resolve({
      ...
      extensionVersion:
        vscode.extensions.getExtension("nellie.nellie")?.packageJSON.version,
    });
  }
  ...
  async getWorkspaceConfigs() {
    ...
    for (const workspaceDir of workspaceDirs) {
      ...
      for (const [filename, type] of files) {
        if (type === vscode.FileType.File && filename === ".nellierc.json") {
  ...
  async updateNellieCredentials(auth: PearAuth): Promise<void> {
    await this.ideUtils.updateNellieCredentials(auth);
  }
